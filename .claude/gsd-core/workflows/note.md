...
- **Global scope**: `/home/user/toofies/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/toofies/.claude/notes/`)
...
2. Glob `/home/user/toofies/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/toofies/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/toofies/.claude/notes/` — works in any directory
